...
export function setFormItemId(item: WForm.Schema.Item, index: number, value: boolean) {
  const id = generateFormItemId(item, index)

  formIdMap.value.set(id, value)
}

export function getFormItemId(item: WForm.Schema.Item, index: number) {
  const id = generateFormItemId(item, index)

  return formIdMap.value.get(id)
}

export function toggleFormItemId(item: WForm.Schema.Item, index: number) {
  const id = generateFormItemId(item, index)
